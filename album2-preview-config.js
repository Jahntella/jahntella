...
(()=>{
  ...
  const addHomepageVisualizers=()=>{
    ...
    g.querySelectorAll(':scope > article').forEach(card=>{
      ...
      if(title==='Midnight Rodeo'||title==='Redline'||title==='Smoke Show'||title==='Chasing Me')card.remove();
    });
    ...
    if(ref?.parentNode){ref.parentNode.insertBefore(midnight,ref.nextSibling);ref.parentNode.insertBefore(redline,midnight.nextSibling);ref.parentNode.insertBefore(smoke,redline.nextSibling);ref.parentNode.insertBefore(chasing,smoke.nextSibling);}else{g.append(midnight,redline,smoke,chasing);}
  ...
  const orderAestheticsByAlbum=()=>{
    ...
    const order=[
      ...
      'sweet-dreams-cover.webp','we-are-1-cover.webp','boots-smile-attitude-cover.webp','midnight-rodeo-cover.webp','redline-cover.webp','smoke-show-cover.webp','chasing-me-cover.webp'
    ];
  ...
  const run=()=>{
    ...
    if(c)c.innerHTML=c.innerHTML.replace(/\b(?:15|16|17|18) new songs\b/gi,'19 new songs');

    document.querySelectorAll('strong,span,p,h2,h3').forEach(node=>{
      if(node.children.length)return;
      node.textContent=node.textContent.replace(/^(?:17|18)(?=\s*$)/,'19').replace(/\b(?:17|18) original songs\b/gi,'19 original songs');
    });
