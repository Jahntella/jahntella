(() => {
  ...
  const VERSION = "6.2";
  const STORAGE_KEY = "jahntella_v62_profile";
  const LEGACY_STORAGE_KEY = "jahntella_v61_profile";
  ...
  function renderVaultModal(){
    ...
    $(".vault-shell").innerHTML = `${closeButton()}
      <p class="eyebrow">COLLECTION BOOK 2.0</p><h2>The Sweet <em>Vault.</em></h2>
      <p>${profile.cards.length}/12 unique cards · ${Object.values(profile.cardCopies).reduce((a,b)=>a+b,0)} total cards.</p>
  ...
  function finishGame(){
    ...
    unlockAchievement("starCatch",false);unlockCard(6,false);reward(20,5);
  ...
  function init(){
    ...
    applyDayPhase(); setInterval(applyDayPhase,60000); renderLoginStreak();
