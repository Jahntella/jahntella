...
async function shareCurrentCard() {
  ...
  const data = {title:"Jahntella Sweet Card", text, url};

  try {
    if (navigator.share) {
      await navigator.share(data);
      showToast("Sweet Drop shared! 💖");
      return;
    }

    const copied = await copyShareText(`${text} ${url}`);
    showToast(copied ? "Sweet Drop message copied! 💖" : text);
  } catch (error) {
    if (error?.name === "AbortError") return;

    try {
      const copied = await copyShareText(`${text} ${url}`);
      showToast(copied ? "Sweet Drop message copied! 💖" : "Share message ready below.");
    } catch (copyError) {
      console.error(copyError);
      window.prompt("Copy and share your Sweet Drop:", `${text} ${url}`);
    }
  }
}
