(()=>{
  ...
  const values=()=>({
    subject:$('subject').value.trim(),preheader:$('preheader').value.trim(),headline:$('headline').value.trim(),
    message:$('message').value.trim(),buttonText:$('buttonText').value.trim(),buttonUrl:$('buttonUrl').value.trim(),imageUrl:$('imageUrl').value.trim()
  });
  ...
  const showPreview=()=>{
    const data=values();
    if(!data.subject||!data.headline||!data.message)return setStatus('Add a subject, headline, and message first.',true);
    $('previewFrame').srcdoc=buildEmail(data);$('previewCard').hidden=false;$('previewCard').scrollIntoView({behavior:'smooth',block:'start'});
  };
  ...
  async function init(){
    const {data:{session}}=await client.auth.getSession();
    if(!session){$('authStatus').textContent='Please sign in through the normal Sweetie Access flow first.';return;}
  ...
  $('preview').addEventListener('click',showPreview);$('closePreview').addEventListener('click',()=>$('previewCard').hidden=true);$('refreshCount').addEventListener('click',refreshCount);$('sendTest').addEventListener('click',sendTest);$('campaignForm').addEventListener('submit',sendCampaign);$('signOut').addEventListener('click',async()=>{await client.auth.signOut();location.reload()});
  client.auth.onAuthStateChange((_event,session)=>{if(!session)location.reload()});
  init();
})();
