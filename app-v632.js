(() => {
  ...
  const VERSION = "6.3.2";
  const STORAGE_KEY = "jahntella_v632_profile";
  const LEGACY_STORAGE_KEY = "jahntella_v631_profile";
  ...
  // Official Jahntella Sweet Vault Series 1 — premium music and visual lyric cards.
  const cards = [
    ["👑","Bubblegum Queen"],["🍭","Fun Dipp"],["💋","Pink Lips"],["🐶","Candy Rebel"],
    ["💖","Neon Sweetheart"],["🌃","Donut District"],["🎙️","Melody Studio"],["🌙","Sparkle Lake"],
    ["☕","Pink Café"],["☁️","Cotton Candy Clouds"],["🍎","Stay Sweet"],["🌈","XO Sweetie"]
  ];

  const cardMeta = [
    {number:"001",rarity:"legendary",source:"Signature Card",image:"cards/sv-001-bubblegum-queen.png",subtitle:"Rule the Sweet Era",quote:"Own your sparkle. Rule your story."},
    {number:"002",rarity:"ultra",source:"Fun Dipp · Music Era",image:"cards/sv-002-fun-dipp.png",subtitle:"Dip Into the Sweet",quote:"Dip in, glow up, stay sweet."},
    {number:"003",rarity:"ultra",source:"Pink Lips · Music Era",image:"cards/sv-003-pink-lips.png",subtitle:"The Kiss of the Era",quote:"Say it sweet. Mean it boldly."},
    {number:"004",rarity:"epic",source:"Sweetville Stories",image:"cards/sv-004-candy-rebel.png",subtitle:"Sweet With an Edge",quote:"Break the rules. Keep the heart."},
    {number:"005",rarity:"epic",source:"Neon Lounge",image:"cards/sv-005-neon-sweetheart.png",subtitle:"Light Up the Night",quote:"Glow like the night was made for you."},
    {number:"006",rarity:"rare",source:"Sweetville Visual",image:"cards/sv-006-donut-district.png",subtitle:"Cruise the Sweet Side",quote:"Take the sweetest route."},
    {number:"007",rarity:"rare",source:"Behind the Music",image:"cards/sv-007-melody-studio.png",subtitle:"Where the Magic Starts",quote:"Turn the feeling into a song."},
    {number:"008",rarity:"rare",source:"Sweetville Visual",image:"cards/sv-008-sparkle-lake.png",subtitle:"Make a Wish",quote:"Carry your own light."},
    {number:"009",rarity:"rare",source:"Sweetville Visual",image:"cards/sv-009-pink-cafe.png",subtitle:"Start the Day Sweet",quote:"Begin softly. Dream boldly."},
    {number:"010",rarity:"rare",source:"Sweet Dreams",image:"cards/sv-010-cotton-candy-clouds.png",subtitle:"Dream in Pink",quote:"Rest is part of the magic."},
    {number:"011",rarity:"rare",source:"Sweet Lessons",image:"cards/sv-011-stay-sweet.png",subtitle:"A Lesson Worth Sharing",quote:"Stay sweet. Stand strong."},
    {number:"012",rarity:"secret",source:"Secret Card",image:"cards/sv-012-xo-sweetie-secret.png",subtitle:"The Hidden Kiss",quote:"XO, Sweetie. You found the secret."}
  ];
  const rarityNames={rare:"Rare",epic:"Epic",ultra:"Ultra Rare",legendary:"Legendary",secret:"Secret Rare"};
  const rarityWeights={rare:48,epic:26,ultra:15,legendary:8,secret:3};
  ...
  function renderBinder(){
    ...
    binder.innerHTML = cards.map((card,i)=>`
      <div class="mini-card premium-mini-card ${profile.cards.includes(i)?"":"locked"} rarity-border-${cardMeta[i].rarity}">
        ${profile.cards.includes(i)
          ? `<img src="${cardMeta[i].image}" alt="${card[1]} collectible card"><span class="card-foil-layer"></span>`
          : `<div class="locked-card-back"><img src="cards/sweet-vault-card-back.png" alt="Locked Jahntella card"></div>`}
        <small>${profile.cards.includes(i)?`#${cardMeta[i].number} · ${card[1]}`:"UNDISCOVERED"}</small>
  ...
  function renderVaultModal(){
    ...
    $(".vault-shell").innerHTML = `${closeButton()}
      ...
      ${recent.length?`<h3>Recently Collected</h3><div class="vault-recent-strip">${recent.map(i=>`<div class="recent-vault-card premium-recent-card"><img src="${cardMeta[i].image}" alt="${cards[i][1]}"><strong>${cards[i][1]}</strong><small class="rarity-${cardMeta[i].rarity}">${rarityNames[cardMeta[i].rarity]}</small></div>`).join("")}</div>`:""}
      <div class="vault-controls">${["all","rare","epic","ultra","legendary","secret"].map(r=>`<button class="vault-filter ${vaultFilter===r?"active":""}" data-vault-filter="${r}">${r==="all"?"All Cards":rarityNames[r]}</button>`).join("")}</div>
      <div class="vault-book">${filtered.map(({c,i})=>`<article class="vault-card premium-vault-card ${profile.cards.includes(i)?"":"locked"} rarity-border-${cardMeta[i].rarity}">
        ${profile.cards.includes(i)?`<span class="copy-count">×${profile.cardCopies[i]||1}</span>`:""}
        <div class="vault-card-art">
          <img src="${profile.cards.includes(i)?cardMeta[i].image:"cards/sweet-vault-card-back.png"}" alt="${profile.cards.includes(i)?c[1]:"Locked Jahntella card"}">
          ${profile.cards.includes(i)?`<span class="card-foil-layer"></span>`:""}
        </div>
        <strong>${profile.cards.includes(i)?`#${cardMeta[i].number} · ${c[1]}`:"UNDISCOVERED"}</strong>
        <small class="card-rarity rarity-${cardMeta[i].rarity}">${profile.cards.includes(i)?rarityNames[cardMeta[i].rarity]:"LOCKED"}</small>
        ${profile.cards.includes(i)?`<small>${cardMeta[i].source}</small><blockquote>${cardMeta[i].quote}</blockquote>`:""}
      </article>`).join("")}</div>`;
  }
  ...
  function weightedCard(minRare=false){
    const pool=cards.map((_,i)=>i).filter(i=>!minRare||["rare","epic","ultra","legendary","secret"].includes(cardMeta[i].rarity));
  ...
  function renderPackModal(){
    ...
    $(".pack-shell").innerHTML=`${closeButton()}<div class="pack-opening">
      <div class="pack-stage" id="packStage">
        ...
        ${count>0?`<button class="sweet-pack-display opening-pack premium-foil-pack" id="openPackButton" type="button"><span class="foil-pack-logo">JAHNTELLA</span><b>SWEET ERA</b><small>FIRST EDITION · TAP TO OPEN</small><i>✦</i><span class="foil-pack-shine"></span></button><p>You have <strong>${count}</strong> premium pack${count===1?"":"s"}.</p>`:`<div class="sweet-pack-display opening-pack premium-foil-pack sold-out"><span class="foil-pack-logo">JAHNTELLA</span><b>NO PACKS</b><small>VISIT THE PACK SHOP</small><i>✦</i><span class="foil-pack-shine"></span></div><button class="button primary" data-open-shop-inline type="button">Buy a Pack · 35 Coins</button>`}
      </div></div>`;
    openModal("pack");
  }
  ...
  function openSweetPack(){
    if(packOpening || (profile.packs||0)<1)return;
    const stage=$("#packStage");
    const button=$("#openPackButton");
    ...
    setTimeout(()=>{
      ...
      stage.innerHTML=`<p class="eyebrow">TAP EACH CARD TO REVEAL</p><h2>Your Sweet <em>Pulls.</em></h2>
        <div class="reveal-row">${results.map(({i,isNew})=>`<button class="reveal-card" data-reveal-card type="button"><span class="card-inner">
          <span class="card-face card-back premium-card-back"><img src="cards/sweet-vault-card-back.png" alt="Jahntella Sweet Vault card back"></span><span class="card-face card-front premium-card-front ${cardMeta[i].rarity}">
            ${!isNew?'<b class="duplicate-badge">DUPLICATE</b>':""}
            <img src="${cardMeta[i].image}" alt="${cards[i][1]} — Jahntella collectible card">
            <span class="card-foil-layer"></span>
            <span class="premium-card-caption"><strong>#${cardMeta[i].number} · ${cards[i][1]}</strong><small class="rarity-${cardMeta[i].rarity}">${rarityNames[cardMeta[i].rarity]}</small></span>
  ...
  function renderPackShop(){
    $(".packshop-shell").innerHTML=`${closeButton()}<p class="eyebrow">SWEET COIN SHOP</p><h2>Pack <em>Shop.</em></h2>
      ...
      <div><h3>Jahntella Sweet Era Pack</h3><p>Three premium Jahntella visual cards inspired by her music, lyrics, and Sweet Era.</p><button class="button primary" id="buyPack" type="button" ${profile.coins<35?"disabled":""}>Buy for 35 Coins</button></div></div>`;
    openModal("packshop");
  }
  ...
  function initEvents(){
    ...
    document.addEventListener("click",e=>{
      ...
      if(e.target.closest("#startGame"))startGame();
      if(e.target.closest("#claimQuest"))claimQuest();
      if(e.target.closest("#openPackButton"))openSweetPack();
      if(e.target.closest("#buyPack"))buyPack();
      if(e.target.closest("#startDonutGame"))startDonutGame();
      if(e.target.closest("#startMelodyGame"))startMelodyGame();
