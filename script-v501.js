...
function cardBackMarkup(card) {
  return `
    <span class="official-card-back-art">
      <img src="assets/cards/sweet-vault-card-back.png" alt="Sweet Vault collectible card back">
      <span class="official-card-shine" aria-hidden="true"></span>
    </span>`;
}
  ...
  if (collectionGrid) {
    collectionGrid.innerHTML = SWEET_SURPRISES.map((card) => {
      if (!collection.includes(card.id)) {
        return `<div class="vault-card-slot locked" data-number="#${card.number}" aria-label="Locked card number ${card.number}"><img src="assets/cards/sweet-vault-card-back.png" alt="Locked Sweet Vault card"></div>`;
      }
