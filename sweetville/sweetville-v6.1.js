...
;(()=>{
 const sky=document.getElementById("motionSky");
 if(sky){
  const icons=["✦","♡","✨","🌸"];
  sky.innerHTML=Array.from({length:28},(_,i)=>`<span style="left:${-10-Math.random()*20}%;top:${Math.random()*100}%;animation-duration:${18+Math.random()*24}s;animation-delay:${-Math.random()*30}s">${icons[i%icons.length]}</span>`).join("");
 }
})();
